  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const endpoint = privacyEnabled
        ? `${baseUrl}/medical-records/search`
        : `${baseUrl}/raw/medical-records/search`;
    ...
    <Box>
      <Card withBorder shadow="sm" radius="md" p="xl" mb="xl"
        ...
        <Group justify="space-between" mb="lg" align="center">
          <Title order={4} style={{ color: '#f8fafc' }}>Search Medical Records</Title>
        ...
        <Grid>
          ...
          <Grid.Col span={{ base: 12, md: 6 }}>
            <TextInput label="Doctor IDs" placeholder="e.g. 1, 3" value={filters.doctorIds}
              onChange={(e) => setFilters({ ...filters, doctorIds: e.currentTarget.value })}
              styles={{ label: { color: '#cbd5e1' } }} />
          </Grid.Col>
          <Grid.Col span={{ base: 12, md: 6 }}>
            <Group grow>
              <TextInput type="date" label="Visit Start Date" value={filters.visitDateStart}
                onChange={(e) => setFilters({ ...filters, visitDateStart: e.currentTarget.value })}
                styles={{ label: { color: '#cbd5e1' } }} />
              <TextInput type="date" label="Visit End Date" value={filters.visitDateEnd}
                onChange={(e) => setFilters({ ...filters, visitDateEnd: e.currentTarget.value })}
                styles={{ label: { color: '#cbd5e1' } }} />
            </Group>
          </Grid.Col>
          <Grid.Col span={{ base: 12, md: 6 }}>
            <Group grow>
              <TextInput type="date" label="Next Visit Start" value={filters.nextVisitStart}
                onChange={(e) => setFilters({ ...filters, nextVisitStart: e.currentTarget.value })}
                styles={{ label: { color: '#cbd5e1' } }} />
              <TextInput type="date" label="Next Visit End" value={filters.nextVisitEnd}
                onChange={(e) => setFilters({ ...filters, nextVisitEnd: e.currentTarget.value })}
                styles={{ label: { color: '#cbd5e1' } }} />
            </Group>
          </Grid.Col>
        </Grid>
      ...
      <Card withBorder shadow="sm" radius="md" p="xl"
        ...
        <Group justify="space-between" mb="md">
          ...
          <Badge color={privacyEnabled ? 'red' : 'green'} variant="light" size="lg"
            ...
            {privacyEnabled ? '/api/medical-records/search' : '/api/raw/medical-records/search'}
          </Badge>
        </Group>
